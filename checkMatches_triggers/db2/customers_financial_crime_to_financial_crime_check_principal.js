...
async function checkprincipaltaxcrimeMatches() {
  ...
  try {
    ...
    for (const { MEG_BIR_TIN_PRINCIPAL: tin } of customers) {
      const crimeQuery = `
        SELECT CRIME_STATUS 
    ...
    if (goodGuyTINs.length > 0) {
      ...
      const getIDsQuery = `
        SELECT MEG_ID 
        FROM MEGAPLANET.CUSTOMERS_TAX 
        WHERE MEG_BIR_TIN_PRINCIPAL IN (${placeholders})
      `;
      ...
      for (const id of goodGuyIDs) {
        const updateQuery = `
          ...
          SET MEG_FIN_PRINCIPAL_CRIME_CLEAR = '0' 
          WHERE MEG_ID = ?
        `;
        await housingConn.query(updateQuery, [id]);
        console.log(`📌 Updated MEG_FIN_PRINCIPAL_CRIME_CLEAR to '0' for MEG_ID: ${id}`);
      }
    } else {
    }
