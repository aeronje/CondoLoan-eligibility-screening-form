...
function logUpdateZeroWithCase(megId, tin, status, updatedAt) {
  const when = updatedAt ? toYMD(updatedAt) : '';
  console.log(
    `MEG_ID ${megId} uses MEG_BIR_TIN_PRINCIPAL ${tin} — confirmed to have a '${status}' financial criminal case; last updated ${when} in DOJ records, which deserves to receive a value of ('0') in megaplanet.customers_financial_crime`
  );
}

function logUpdateZeroNoCase(megId, tin) {
  console.log(
    `MEG_ID ${megId} uses MEG_BIR_TIN_PRINCIPAL ${tin} — confirmed to have no financial criminal case in DOJ records, which deserves to receive a value of ('0') in megaplanet.customers_financial_crime`
  );
}
...
function logUpdateOneConvicted(megId, tin, updatedAt) {
  const when = updatedAt ? toYMD(updatedAt) : '';
  console.log(
    `MEG_ID ${megId} uses MEG_BIR_TIN_PRINCIPAL ${tin} — confirmed to have a 'Convicted' financial criminal case; last updated ${when} in DOJ records, which deserves to receive a value of ('1') in megaplanet.customers_financial_crime`
  );
}
...
async function runCheckMatches() {
  ...
  const updateCrimeClearSql = `
    ...
    SET meg_fin_principal_crime_clear = $1
    WHERE meg_id = $2
  `;

  for (const row of taxRes.rows) {
    const megId = row.meg_id;
    const tin = (row.meg_bir_tin_principal ?? '').toString().trim();
...
setInterval(() => {
  ...
  console.log(`[${now}] ***MEGAPLANET.CUSTOMERS_TAX MARRIAGE CRIME TWO routinary 10 seconds checkMatches***`);
